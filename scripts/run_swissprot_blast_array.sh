#PBS -l nodes=1:ppn=2 -N spblast -j oe
SIZE=500
...
if [ ! -d $PREFIX.BLAST ]; then
 mkdir $PREFIX.BLAST
 bp_dbsplit.pl --size 500 --prefix $PREFIX.BLAST/db $SPECIES.all.maker.proteins.fasta
fi

if [ ! -f $PREFIX.BLAST/$PREFIX.$N.BLASTP ]; then
blastp -query $PREFIX.BLAST/db.$N -db $DB -use_sw_tback \
